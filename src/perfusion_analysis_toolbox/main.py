...
from lib.utils import get_logger, datestr
...
def main():
    """
    Application entrypoint. Parse arguments, load and preprocess data, and compute perfusion parameters.
    """
    print(datestr())
    config = parse_config()
    ...
    RawPerfImg, mask, vessels, origin, spacing, direction = \
        read_signal(config.image_path, config.mask_path, config.vessel_path, ToTensor = config.to_tensor) 
    
    # Calculate perfusion parameters
    calculator = MainCalculator(RawPerfImg, mask, vessels, origin, spacing, direction, config, config.save_path, device)
    calculator.run()
