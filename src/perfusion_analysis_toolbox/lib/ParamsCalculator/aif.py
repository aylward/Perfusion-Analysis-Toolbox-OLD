...
def extract_AIF(AIFslice, mask, vesselSlice, bolusSlice):
    ...
    semiaxisB = semiaxisMag * (
        maxC - minC
    )  # The major axis is along the anterior-posterior direction,
    # i.e., left to right in the images
    ...
    for r in range(nR):
        for c in range(nC):
            if (
                (r - center[1]) ** 2 / (semiaxisA**2)
                + (c - center[0]) ** 2 / (semiaxisB**2)
            ) <= 1:
                ROI[r, c] = 1
                count += 1
    ROI = ROI * mask
    ...
    for r in range(nR):
        for c in range(nC):
            if maskAIF[r, c] == 1:
                ...
                weights = 0.01 + np.exp(
                    -AIFconc
                )  # Weights for the fit calculation.
    ...
    avg_alpha, avg_beta = find_avg_alpha_beta(
        maskAIF, fit_Params, count, nR, nC
    )
    ...
    while True:
        ...
        for r in range(nR):
            for c in range(nC):
                if maskAIF[r, c] == 1:
                    ...
                    weights[TTP - 1] = weights[TTP - 1] / 5
                    weights[TTP + 1] = weights[TTP + 1] / 2
                    fitParams_bat[r, c] = find_params_bat_optim(
                        AIFconc,
                        weights,
                        fitParams_avg,
                        fit_Params_temp[r, c, 0],
                        fit_Params_temp[r, c, 3],
                    )
                    fitParams_ab[r, c] = find_params_alpha_beta_optim(
                        AIFconc,
                        weights,
                        fitParams_bat[r, c],
                        avg_alpha,
        ...
        avg_alpha, avg_beta = find_avg_alpha_beta(
            maskAIF, fit_Params_temp, count, nR, nC
        )
...
def fitGV_peak1(data, weights):
    ...
    A_init = MCdata / np.max(
        GVfunction_peak1([t0_init, alpha_init, beta_init, 1])
    )
    ...
    _MC, TTP = np.max(data), np.argmax(data)
    weights[TTP] /= 10
    weights[TTP - 1] /= 2
    ...
    # Estimator
    p = p0
    least_squares_output = least_squares(
        objFitGV_peak1,
        p,
        bounds=(lb, ub),
        max_nfev=1000,
...
def fitGV_peak1_2(data, weights, param1, param2, t0, A):
    ...
    if A == 0:
        A_init = MCdata / np.max(
            GVfunction_peak1_2([t0_init, 1], param1, param2)
        )
    else:
        A_init = A
    ...
    least_squares_output = least_squares(
        ...
        ftol=1e-8,
        max_nfev=1000,
        args=(data_peak1, weights_peak1, param1, param2),
    )
    GVparameter = np.transpose(least_squares_output.x)
...
def find_params_bat_optim(avg_test, avg_weights, fitParams_test, t0, A):
    ...
    fitParams_test = (
        fitGV_peak1_2(avg_test, avg_weights, param1, param2, t0, A)
    )[0:2]
...
def find_params_alpha_beta_optim(
    avg_test, avg_weights, fitParams_test, alpha, beta
):
    param1 = fitParams_test[0]
    param2 = fitParams_test[1]
    fitParams_test = (
        fitGV_peak1_1(avg_test, avg_weights, param1, param2, alpha, beta)
    )[0:2]
    return fitParams_test
